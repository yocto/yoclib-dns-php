...
class WALLET extends Type{
     ...
     * @throws DNSTypeException
     */
    public static function deserializeFromPresentationFormat(string $data): WALLET{
        $tokens = LineLexer::tokenizeLine($data);
        if(count($tokens)<1){
            throw new DNSTypeException('WALLET record should have at least one character string.');
        }
     ...
     * @throws DNSTypeException
     */
    public static function deserializeFromWireFormat(string $data): WALLET{
        if(strlen($data)===0){
            throw new DNSTypeException('WALLET record should have at least some data.');
        }
        $offset = 0;
