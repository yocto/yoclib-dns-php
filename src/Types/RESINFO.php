...
class RESINFO extends Type{
     ...
     * @throws DNSTypeException
     */
    public static function deserializeFromPresentationFormat(string $data): RESINFO{
        $tokens = LineLexer::tokenizeLine($data);
        if(count($tokens)<1){
            throw new DNSTypeException('RESINFO record should have at least one character string.');
        }
    ...
    public static function deserializeFromWireFormat(string $data): RESINFO{
        if(strlen($data)===0){
            throw new DNSTypeException('RESINFO record should have at least some data.');
        }
        $offset = 0;
